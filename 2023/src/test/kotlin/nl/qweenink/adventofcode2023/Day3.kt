...
class Day3 {
    ...
    fun puzzle1() {
        ...
        for ((lineIndex, line) in lines.withIndex()) {
            ...
            for ((charIndex, char) in line.toCharArray().withIndex()) {
                ...
                if(!char.isDigit() || (charIndex == line.length - 1 && char.isDigit())) {
                    if(numberStr != "") {
                        val startNumberIndex = if (charIndex - numberStr.length - 1 >= 0) charIndex - numberStr.length - 1 else 0
                        val endNumberIndex = if (charIndex <= line.length) charIndex else line.length
                        var foundIt = false
                        if (isSymbol(line[startNumberIndex]) || isSymbol(line[endNumberIndex])) {
                            sum += numberStr.toInt()
                            foundIt = true
                            print(" $numberStr")
                        }
                        if (!foundIt && lineIndex - 1 >= 0) {  //Search line above
                            val lineAbove = lines[lineIndex - 1]
                            for (searchIndex in startNumberIndex..endNumberIndex) {
                                if (isSymbol(lineAbove[searchIndex])) {
                                    sum += numberStr.toInt()
                                    foundIt = true
                                    print(" $numberStr")
                                    break
                        ...
                        if (!foundIt && lineIndex + 1 < lines.size) { //Search line below
                            val lineBelow = lines[lineIndex + 1]
                            for (searchIndex in startNumberIndex..endNumberIndex) {
                                if (isSymbol(lineBelow[searchIndex])) {
                                    sum += numberStr.toInt()
                                    print(" $numberStr")
                                    break
                                }
    ...
    fun puzzle2() {
            ...
            for ((charIndex, char) in line.toCharArray().withIndex()) {
                ...
                if(!char.isDigit() || (charIndex == line.length - 1 && char.isDigit())) {
                    if(numberStr != "") {
                        val startNumberIndex = if (charIndex - numberStr.length - 1 >= 0) charIndex - numberStr.length - 1 else 0
                        val endNumberIndex = if (charIndex <= line.length) charIndex else line.length
                        if (isStar(line[startNumberIndex])) {
                            addToHashMap(hashMap, lineIndex, startNumberIndex, numberStr.toInt())
                            print(" $numberStr")
                        }
                        if (isStar(line[endNumberIndex])) {
                            addToHashMap(hashMap, lineIndex, endNumberIndex, numberStr.toInt())
                            print(" $numberStr")
                        }
                        if (lineIndex - 1 >= 0) {  //Search line above
                            val lineAbove = lines[lineIndex - 1]
                            for (searchIndex in startNumberIndex..endNumberIndex) {
                                if (isStar(lineAbove[searchIndex])) {
                                    addToHashMap(hashMap, lineIndex - 1, searchIndex, numberStr.toInt())
                                    print(" $numberStr")
                                }
                            }
                        }
                        if (lineIndex + 1 < lines.size) { //Search line below
                            val lineBelow = lines[lineIndex + 1]
                            for (searchIndex in startNumberIndex..endNumberIndex) {
                                if (isStar(lineBelow[searchIndex])) {
                                    addToHashMap(hashMap, lineIndex + 1, searchIndex, numberStr.toInt())
                                    print(" $numberStr")
                                }
                            }
        ...
        val sum = hashMap.values
                .filter { it.size >= 2 }
                .sumOf { it.first() * it.last() }
        println(sum)
    }

    private fun addToHashMap(hashMap: HashMap<String, MutableList<Int>>, lineIndex: Int, charIndex: Int, number: Int) {
        val key = "$lineIndex-$charIndex"
        if(hashMap.containsKey(key)) {
            hashMap[key]?.add(number)
        } else {
            hashMap[key] = mutableListOf(number)
        }
    }
    ...
    private var data = """........440...............418..643.....438......740.261......................................727...........................870..............

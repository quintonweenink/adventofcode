...
class Day10 {
    ...
    @Test
    fun puzzle1() {
        val lines = data3.split(System.getProperty("line.separator"))
        val grid = lines.map { it.toCharArray() }

        var sPoint = Pair(0, 0)
        grid.forEachIndexed { lineIndex, line ->
            line.forEachIndexed { pointIndex, point ->
                if (point == 'S') {
                    sPoint = Pair(lineIndex, pointIndex)
                }
            }
        }

        var previousPoint1 = sPoint
        var point1 = getStartPoints(grid, sPoint).first()
        var distance = 1
        while (!(point1.first == sPoint.first && point1.second == sPoint.second)) {
            distance++
            val nextPoint1 = getNextPoint(grid, previousPoint1, point1)
    ...
    fun getStartPoints(grid: List<CharArray>,
                     sPoint: Pair<Int, Int>): List<Pair<Int, Int>> {
        val startPoints = mutableListOf<Pair<Int, Int>>()
        val up = grid[sPoint.first - 1][sPoint.second]
        if(listOf('F', '|', '7').contains(up)) {
            startPoints.add(Pair(sPoint.first - 1, sPoint.second))
        }
        val down = grid[sPoint.first + 1][sPoint.second]
        if(listOf('L', '|', 'J').contains(down)) {
            startPoints.add(Pair(sPoint.first + 1, sPoint.second))
        }
        val left = grid[sPoint.first][sPoint.second - 1]
        if(listOf('F', '-', 'L').contains(left)) {
            startPoints.add(Pair(sPoint.first, sPoint.second - 1))
        }
        val right = grid[sPoint.first][sPoint.second + 1]
        if(listOf('J', '-', '7').contains(right)) {
            startPoints.add(Pair(sPoint.first, sPoint.second + 1))
        }
        return startPoints
    }

    fun getNextPoint(grid: List<CharArray>,
                            previousPoint1: Pair<Int, Int>,
                            point1: Pair<Int, Int>): Pair<Int, Int> {
        if (grid[point1.first][point1.second] == '|') {
            return if (previousPoint1 != Pair(point1.first + 1, point1.second)) {
                Pair(point1.first + 1, point1.second)
            } else {
                Pair(point1.first - 1, point1.second)
            }
        } else if (grid[point1.first][point1.second] == '-') {
            return if (previousPoint1 != Pair(point1.first, point1.second + 1)) {
                Pair(point1.first, point1.second + 1)
            } else {
                Pair(point1.first, point1.second - 1)
            }
        } else if (grid[point1.first][point1.second] == 'J') {
            return if (previousPoint1 != Pair(point1.first - 1, point1.second)) {
                Pair(point1.first - 1, point1.second)
            } else {
                Pair(point1.first, point1.second - 1)
            }
        } else if (grid[point1.first][point1.second] == 'L') {
            return if (previousPoint1 != Pair(point1.first - 1, point1.second)) {
                Pair(point1.first - 1, point1.second)
            } else {
                Pair(point1.first, point1.second + 1)
            }
        } else if (grid[point1.first][point1.second] == 'F') {
            return if (previousPoint1 != Pair(point1.first + 1, point1.second)) {
                Pair(point1.first + 1, point1.second)
            } else {
                Pair(point1.first, point1.second + 1)
            }
        } else if (grid[point1.first][point1.second] == '7') {
            return if (previousPoint1 != Pair(point1.first + 1, point1.second)) {
                Pair(point1.first + 1, point1.second)
            } else {
                Pair(point1.first, point1.second - 1)
    ...
    fun puzzle2() {
        ...
        var previousPoint1 = sPoint
        var point1 = getStartPoints2(grid, sPoint).first()
        var distance = 1
        val loop = mutableListOf(sPoint)
        while (!(point1.first == sPoint.first && point1.second == sPoint.second)) {
            distance++
            loop.add(point1)
            val nextPoint1 = getNextPoint2(grid, previousPoint1, point1)
            previousPoint1 = point1
            point1 = nextPoint1
        }
    ...
    var iPositions = mutableListOf<Pair<Int, Int>>()

    private fun fill(grid: List<CharArray>, loop: List<Pair<Int, Int>>, iPosition: Pair<Int, Int>) {
        if (!loop.contains(Pair(iPosition.first,iPosition.second))) {
            grid[iPosition.first][iPosition.second] = 'I'
        } else return

        if (grid[iPosition.first - 1][iPosition.second] != 'I') {
            fill(grid, loop, Pair(iPosition.first + 1, iPosition.second))
        }
        if (grid[iPosition.first - 1][iPosition.second] != 'I') {
            fill(grid, loop, Pair(iPosition.first - 1, iPosition.second))
        }
        if (grid[iPosition.first][iPosition.second + 1] != 'I') {
            fill(grid, loop, Pair(iPosition.first, iPosition.second + 1))
        }
        if (grid[iPosition.first][iPosition.second - 1] != 'I') {
            fill(grid, loop, Pair(iPosition.first, iPosition.second - 1))
        }
    ...
    fun getStartPoints2(grid: List<CharArray>,
                       sPoint: Pair<Int, Int>): List<Pair<Int, Int>> {
        val startPoints = mutableListOf<Pair<Int, Int>>()
        val up = grid[sPoint.first - 1][sPoint.second]
        if(listOf('F', '|', '7').contains(up)) {
    ...
    fun getNextPoint2(grid: List<CharArray>,
                     previousPoint1: Pair<Int, Int>,
                     point1: Pair<Int, Int>): Pair<Int, Int> {
        if (grid[point1.first][point1.second] == '|') {
            return if (previousPoint1 != Pair(point1.first + 1, point1.second)) {
                markGrid(grid, point1, Direction.DOWN)
                Pair(point1.first + 1, point1.second)
            } else {
                markGrid(grid, point1, Direction.UP)
                Pair(point1.first - 1, point1.second)
            }
        } else if (grid[point1.first][point1.second] == '-') {
            return if (previousPoint1 != Pair(point1.first, point1.second + 1)) {
                markGrid(grid, point1, Direction.RIGHT)
                Pair(point1.first, point1.second + 1)
            } else {
                markGrid(grid, point1, Direction.LEFT)
                Pair(point1.first, point1.second - 1)
            }
        } else if (grid[point1.first][point1.second] == 'J') {
            return if (previousPoint1 != Pair(point1.first - 1, point1.second)) {
    ...
    fun markGrid(grid: List<CharArray>, point: Pair<Int, Int>, direction: Direction) {
        val left = when (direction) { //right
            ...
            else -> Pair(point.first + 1, point.second)
        }
...
.LJ..."""
    ...
    val data = """FL7L-F7FJF-7FFF7FF|-F-F|JF--|.|-J-7.F--FJF|-7.F.-FF7.-F7-L-|FF-F-|-7.-F7-FF-|7J-J.F-J7.FF7..L7F|7F-7JF-L7F.|--FL7F---|.FFF-L7FL--L777L-.FF7F
